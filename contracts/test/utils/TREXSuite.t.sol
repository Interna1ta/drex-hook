...
contract TREXSuite is Test {
    ...
    address public identityRegistryStorageAgent =
        makeAddr("identityRegistryStorageAgent"); // Agent in charge of register new identities directly to the storage registry
    ...
    function deployTREXFactory() internal {
        ...
        address claimTopicsRegistryImplementation = deployArtifact(
            "out/ClaimTopicsRegistry.sol/ClaimTopicsRegistry.json",
            hex""
        );
        address trustedIssuersRegistryImplementation = deployArtifact(
            "out/TrustedIssuersRegistry.sol/TrustedIssuersRegistry.json",
            hex""
        );
        address identityRegistryStorageImplementation = deployArtifact(
            "out/IdentityRegistryStorage.sol/IdentityRegistryStorage.json",
            hex""
        );
        address identityRegistryImplementation = deployArtifact(
            "out/IdentityRegistry.sol/IdentityRegistry.json",
            abi.encode(identityRegistryStorageImplementation)
        );
        address tokenImplementation = deployArtifact(
        ...
        identityRegistryStorage = IERC3643IdentityRegistryStorage(
            deployArtifact(
                "out/IdentityRegistryStorageProxy.sol/IdentityRegistryStorageProxy.json",
                abi.encode(trexIA)
            )
        );
        IAgentRole(address(identityRegistryStorage)).addAgent(
            identityRegistryStorageAgent
        );

        // Deploy ONCHAINID
        address identityImplementation = deployArtifact(
            "out/Identity.sol/Identity.json",
            abi.encode(deployer, true)
        );
        identityIA = IImplementationAuthority(
            deployArtifact(
                "out/ImplementationAuthority.sol/ImplementationAuthority.json",
                abi.encode(identityImplementation)
            )
        );
        identityFactory = IIdFactory(
            deployArtifact(
                "out/IdFactory.sol/IdFactory.json",
                abi.encode(identityIA)
            )
        );

        // Deploy TREXFactory
        trexFactory = ITREXFactory(
            deployArtifact(
                "out/TREXFactory.sol/TREXFactory.json",
                abi.encode(trexIA, identityFactory)
            )
        );
    ...
    function deployTSTTokenSchenario() internal {
        ...
        TSTClaimIssuerIdentity = IClaimIssuer(
            deployArtifact(
                "out/ClaimIssuer.sol/ClaimIssuer.json",
                abi.encode(TSTClaimIssuerAddr)
            )
        );
        TSTClaimIssuerIdentity.addKey(
            keccak256(abi.encode(TSTClaimIssuerAddr)),
            3,
            1
        );
        vm.stopPrank();
        ...
        vm.startPrank(deployer);
        TSTContracts.trustedIssuersRegistry.addTrustedIssuer(
        ...
        aliceIdentity = IIdentity(
            deployArtifact(
                ...
                abi.encode(identityIA, aliceAddr)
            )
        );
        vm.stopPrank();
        ...
        charlieIdentity = IIdentity(
            deployArtifact(
                "out/IdentityProxy.sol/IdentityProxy.json",
                abi.encode(identityIA, charlieAddr)
            )
        );
        vm.stopPrank();
        ...
        IAgentRole(address(TSTContracts.identityRegistry)).addAgent(
            address(TSTTokenAgent)
        );
        IAgentRole(address(TSTContracts.identityRegistry)).addAgent(
        ...
        countries[0] = 42;
        countries[1] = 666;
        TSTContracts.identityRegistry.batchRegisterIdentity(
            addrs,
            identities,
            countries
        );
        ...
        ClaimData memory claimForAlice = ClaimData(
            aliceIdentity,
            topics[0],
            "Alice public data!"
        );
        ClaimData memory claimForBob = ClaimData(
            bobIdentity,
            topics[0],
            "Bob public data!"
        );
        bytes memory signatureAliceClaim = signClaim(
            claimForAlice,
            TSTClaimIssuerKey
        );
        bytes memory signatureBobClaim = signClaim(
            claimForBob,
            TSTClaimIssuerKey
        );
        ...
        vm.startPrank(TSTTokenAgent);
        (bool success, ) = TSTContracts.agentManager.call(
            abi.encodeWithSignature("addAgentAdmin(address)", TSTTokenAdmin)
        );
        vm.stopPrank();
        vm.startPrank(deployer);
        IAgentRole(address(TSTContracts.token)).addAgent(
            TSTContracts.agentManager
        );
        IAgentRole(address(TSTContracts.identityRegistry)).addAgent(
            TSTContracts.agentManager
        );
    ...
    function signClaim(
        ...
        bytes32 dataHash = keccak256(
            abi.encode(claim.identity, claim.topic, claim.data)
        );
    ...
    function deployToken(
        ...
        tokenContracts.identity = IIdentity(
            deployArtifact(
                "out/IdentityProxy.sol/IdentityProxy.json",
                abi.encode(identityIA, tokenIssuer)
            )
        );
        ...
        tokenContracts.claimTopicsRegistry = IERC3643ClaimTopicsRegistry(
            deployArtifact(
                "out/ClaimTopicsRegistryProxy.sol/ClaimTopicsRegistryProxy.json",
                abi.encode(trexIA)
            )
        );
        tokenContracts.trustedIssuersRegistry = IERC3643TrustedIssuersRegistry(
            deployArtifact(
                "out/TrustedIssuersRegistryProxy.sol/TrustedIssuersRegistryProxy.json",
                abi.encode(trexIA)
            )
        );
        tokenContracts.compliance = IERC3643Compliance(
            deployArtifact(
                "out/DefaultCompliance.sol/DefaultCompliance.json",
                hex""
            )
        ); // Mock compliance canTransfer always true
        ...
        tokenContracts.agentManager = deployArtifact(
            "out/AgentManager.sol/AgentManager.json",
            abi.encode(tokenContracts.token)
        );
        vm.stopPrank();
        ...
        identityRegistryStorage.bindIdentityRegistry(
            address(tokenContracts.identityRegistry)
        );
