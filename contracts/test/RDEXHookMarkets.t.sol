...
contract RDEXHookMarketsTest is Test, TREXSuite, Deployers {
    ...
    function setUp() public {
        ...
        address dynamicFeeHookAddress = address(
            (uint160(makeAddr("RDEXDynamicFeeHook")) & ~Hooks.ALL_HOOK_MASK) |
        ...
        public
    {
        ...
        ERC20RDEXWrapper erc20Wrapper = hook.s_ERC3643ToERC20WrapperInstances(
    ...
    function test_tokenOwnersShouldBeAbleToModifyLiquidity() public {
        Currency _currency0;
        Currency _currency1;
        if (address(refCurrency) < address(TSTContracts.token)) {
            _currency0 = Currency.wrap(address(refCurrency));
        ...
        console.log(
            "Alice balance of refCurrency before deposit: %18e",
        ...
        console.log(
            "Alice balance of refCurrency after deposit: %18e",
            refCurrency.balanceOf(aliceAddr)
        );
        console.log(
        ...
        console.log(
            "Alice balance of refCurrency after remove 5 ether of liquidity: %18e",
            refCurrency.balanceOf(aliceAddr)
        );
        console.log(
        ...
        console.log(
            "Alice balance of refCurrency after remove 10 ether of liquidity: %18e",
            refCurrency.balanceOf(aliceAddr)
        );
        console.log(
    ...
    function test_tokenOwnersShouldBeAbleToSwap() public {
        Currency _currency0;
        Currency _currency1;
        bool refIs0 = address(refCurrency) < address(TSTContracts.token);
        if (refIs0) {
            _currency0 = Currency.wrap(address(refCurrency));
            _currency1 = Currency.wrap(address(TSTContracts.token));
        } else {
        ...
        // zeroForOne exact input
        vm.startPrank(aliceAddr);
        console.log("/*======== ZeroForOne exact input ========*/");
        console.log(
            "Alice balance of refCurrency before swap:",
            refCurrency.balanceOf(aliceAddr)
        );
        console.log(
            "Alice balance of ERC3643 before swap:",
            TSTContracts.token.balanceOf(aliceAddr)
        );
        if (refIs0) {
            console.log("Alice swaps 100 of refCurrency for x ERC3643");
        } else {
            console.log("Alice swaps 100 of x ERC3643 for refCurrency");
        }
        uint256 aliceCurrency0BalanceBefore = key.currency0.balanceOf(
            aliceAddr
        );
        hook.swap(
            key,
        ...
        assertEq(
            key.currency0.balanceOf(aliceAddr),
            aliceCurrency0BalanceBefore - 100
        );
        console.log(
            "Alice balance of refCurrency after swap:",
            refCurrency.balanceOf(aliceAddr)
        );
        console.log(
            "Alice balance of ERC3643 after swap:",
            TSTContracts.token.balanceOf(aliceAddr)
        );
        vm.stopPrank();
        ...
        console.log(
            "Bob balance of refCurrency before swap:",
            refCurrency.balanceOf(bobAddr)
        );
        console.log(
            "Bob balance of ERC3643 before swap:",
            TSTContracts.token.balanceOf(bobAddr)
        );
        if (refIs0) {
            console.log("Bob swaps x of refCurrency for 100 of ERC3643");
        } else {
        ...
        assertEq(
            key.currency1.balanceOf(bobAddr),
            bobCurrency1BalanceBefore + 100
        );
        console.log(
            "Bob balance of refCurrency after swap:",
            refCurrency.balanceOf(bobAddr)
        );
        console.log(
            "Bob balance of ERC3643 after swap:",
            TSTContracts.token.balanceOf(bobAddr)
        );
        vm.stopPrank();
        ...
        uint256 aliceCurrency1BalanceBefore = key.currency1.balanceOf(
            aliceAddr
        );
        hook.swap(
            key,
        ...
        assertEq(
            key.currency1.balanceOf(aliceAddr),
            aliceCurrency1BalanceBefore - 100
        );
        console.log(
            "Alice balance of refCurrency after swap:",
            refCurrency.balanceOf(aliceAddr)
        ...
        assertEq(
            key.currency0.balanceOf(bobAddr),
            bobCurrency0BalanceBefore + 100
        );
        console.log(
            "Bob balance of refCurrency after swap:",
            refCurrency.balanceOf(bobAddr)
