...
import {Pausable} from "@openzeppelin@v5.1.0/security/Pausable.sol";
import {ReentrancyGuard} from "@openzeppelin@v5.1.0/security/ReentrancyGuard.sol";
...
contract ServiceManager is IServiceManager, Ownable, Pausable, ReentrancyGuard {
    /* ================== STATE VARS =================== */
    ...
    function deregisterOperator(address _operator) external nonReentrant {
        ...
        address[] public operatorList = s_operatorList;
        for (uint256 i = 0; i < operatorList.length; i++) {
