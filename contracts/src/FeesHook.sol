...
contract FeesHook is BaseHook, Ownable {
    /* ==================  TYPES =================== */
    ...
    mapping(address ERC3643 => ERC20RDEXWrapper)
        public s_ERC3643ToERC20WrapperInstances;
    ...
    function modifyLiquidity(
        ...
        if (
            address(
                s_ERC3643ToERC20WrapperInstances[Currency.unwrap(key.currency0)]
            ) != address(0)
        ) {
            callBackData.token = IERC3643(Currency.unwrap(key.currency0));
            callBackData.erc20Wrapper = s_ERC3643ToERC20WrapperInstances[
                Currency.unwrap(key.currency0)
            ];
            callBackData.refCurrencyAddr = Currency.unwrap(key.currency1);
        } else if (
            address(
                s_ERC3643ToERC20WrapperInstances[Currency.unwrap(key.currency1)]
            ) != address(0)
        ) {
    ...
    function beforeInitialize(
        ...
        try
            IERC165(Currency.unwrap(_key.currency0)).supportsInterface(
                type(IERC3643).interfaceId
            )
        returns (bool isERC3643) {
            currency0IsERC3643 = isERC3643;
        } catch {}
        try
            IERC165(Currency.unwrap(_key.currency1)).supportsInterface(
                type(IERC3643).interfaceId
            )
        returns (bool isERC3643) {
        ...
        if (currency0IsERC3643) {
            ...
            identity = IIdentity(
                s_identityRegistryStorage.storedIdentity(
                    callBackData.refCurrencyAddr
                )
            );
        ...
        callBackData.erc20Wrapper = ERC20RDEXWrapper(
            i_erc20WrapperImplementation.clone()
        );
        ...
        )
    {
        ...
        (
            ...
            poolManager,
            s_poolIdToWrapperPoolKey[_poolId].toId(),
            positionKey
        );
    }
    ...
    function _sortCurrencies(
        address _currencyAAddr,
        address _currencyBAddr
    ...
    function _unlockCallback(
        ...
        if (callBackData.initializePool) {
            ...
            poolManager.mint(
                address(this),
                uint256(uint160(address(callBackData.erc20Wrapper))),
                MAX_SUPPLY
            );
            // Settle delta
            Currency currency = Currency.wrap(
                address(callBackData.erc20Wrapper)
            );
            currency.settle(poolManager, address(this), MAX_SUPPLY, false);
            (Currency _WCurrency0, Currency _WCurrency1) = _sortCurrencies(
                callBackData.refCurrencyAddr,
                address(callBackData.erc20Wrapper)
            );
            ...
            s_poolIdToWrapperPoolKey[
                callBackData.poolKey.toId()
            ] = wrapperPoolKey;
        } else if (callBackData.modifyLiquidity) {
            IPoolManager.ModifyLiquidityParams memory params = callBackData
            ...
            params.liquidityDelta = callBackData
                .modifyLiquidityParams
                .liquidityDelta;
            params.salt = keccak256(
                abi.encodePacked(
                    callBackData.user,
                    callBackData.modifyLiquidityParams.salt
                )
            );

            PoolKey memory wrapperPoolKey = s_poolIdToWrapperPoolKey[
                callBackData.poolKey.toId()
            ];
            ...
            if (
                Currency.unwrap(wrapperPoolKey.currency0) ==
                address(callBackData.erc20Wrapper) &&
                Currency.unwrap(wrapperPoolKey.currency1) ==
                callBackData.refCurrencyAddr
            ) {
                ...
                if (delta0 < 0) {
                    ...
                    callBackData.token.transferFrom(
                        callBackData.user,
                        address(this),
                        uint256(-delta0)
                    );
    ...
    function swap(
        ...
        if (msg.value > 0) {
            (bool isReducedFee, address destinationChain) = abi.decode(
                _hookData,
                (bool, address)
            );
            _hookData = abi.encode(isReducedFee, destinationChain, msg.value);
        }

        IPoolManager(poolManager).swap(_key, _params, _hookData);

        //q put bridging logic here or have standalone deposit function and just do it in afterSwap?
    }
}
